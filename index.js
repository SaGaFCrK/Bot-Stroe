...
let owners = ''//ايدي الي هيتحول له الكريديت
let owner;
const
  updateAdmins = () => {
    owner = db.get("admins")
    owners = ''//ايدي الي هيتحول له الكريديت
    owner.unshift('')//اي دي حقك
  }
updateAdmins()
...
client.on("ready", () => {
 ...
 client.on('guildCreate', guild => {
  ...
  let textList = [`${prefix}Stock`, `${prefix}help`, "BY xyous"]
  client.user.setPresence({ activities: [{ name: `${prefix}help` }], status: 'idle' })
